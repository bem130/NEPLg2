...
オフサイドルールはブロック式か引数か複行文字列リテラルの2種類に分けられる
...
### 文字列
文字列はUTF-8です
文字列リテラルは、単行のものと複行のものを用意します
複行文字列リテラル(multi line string literal)はmlstr:に続き、インデントを1つ増やして`##: `の後ろに文字列を書きます
複行文字列リテラルは、行間に`\n`を挿入し、末尾には挿入しない
複行文字列リテラルは、Raw文字列で、エスケープ処理は行わない 行末のtrimも行わない
str: 借用ビュー (非所有),String: 所有 とする
```
<str> "hello\nworld!"; // 単行文字列リテラル
<str> mlstr: // 複行文字列リテラル
    ##: hello
